...
체결 스냅샷(ka10076 `get_filled_orders`)을 (주문번호, 종목코드) 복합키별로
누적 집계해, 원장의 같은 날 (order_id, stk_cd)별 누적
`executed_quantity`/`total_krw`와 diff한다. 부족분(브로커 누적 > 원장
누적)만 그 차액을 새 행으로 append한다 — 원장에 order_id가 아예 없는
주문도 이 diff에서 자연히 "전량 부족"으로 잡혀 신규 append된다.

집계 키가 `ord_no` 단독이 아니라 (ord_no, stk_cd) 복합인 이유:
`pending_order_tracker.PendingOrderTracker.apply_fills`가 이미 확립한 F3
M1a 계약과 동일하다 — 브로커는 주문번호를 재사용할 수 있어(문서화된
사실, 특히 날짜를 넘나들며), 같은 ord_no가 서로 다른 종목의 체결을
가리킬 수 있다. ord_no만으로 집계하면 그 두 종목의 수량/금액이 하나로
합쳐져 diff 산술이 오염된다(리뷰 지적).
...
원장 자신뿐이다 — 같은 종목의 `entry_or_exit="entry"` 행 **전체**의
수량가중평균가(`SUM(price*executed_quantity)/SUM(executed_quantity)`)를
최선 추정 진입가로 사용해 `record_kr_realized_pnl`을 함께 기록한다. 이는
**근사치(비-FIFO)**다 — 이 코드베이스 어디에도 로트 단위 FIFO 추적은
없고(포지션도 단일 블렌디드 평단만 추적), 이 함수는 코디네이터의
실제 포지션 평단에 접근할 수 없는 백스톱 전용 경로이므로 원장에 남은
entry 행들의 블렌디드 평균이 낼 수 있는 최선의 근사다(정확한 경로는
포지션 평단 기반의 `_apply_sell_position_delta`). 단일 최신 entry 행만
쓰면 멀티랏(예: 100주@200,000 + 50주@210,000 진입 후 80주 매도)에서
최신 행 가격만 반영해 오차가 커진다 — 가중평균은 그 오차를 실질적으로
줄인다. entry 행을 원장에서 전혀 찾을 수 없으면(포지션이 이 원장이
생기기 전에 열렸거나 이미 완전히 정리된 경우) 실현손익 기록은
건너뛴다 — E1-2가 `_apply_sell_position_delta`에서 확립한 "진입가 소스
없음 → 실현손익 스킵, 원장 기록은 계속" 컨벤션과 동일하다.
...
# 진입가 가중평균 계산 시 종목당 훑는 원장 행 수. "전체" entry 행을
# 반영하려는 취지이므로 넉넉히 잡는다(단일 종목이 하루/기간 동안 쌓을
# 수 있는 왕복 매매 행 수를 comfortably 상회 — eod_snapshot의
# kr_realized_pnl 스캔 한도 500과 동일한 정신).
_ENTRY_LOOKUP_LIMIT = 500
...
async def _lookup_entry_price(storage: Any, stk_cd: str) -> Optional[float]:
    """종목의 entry(매수) 원장 행 전체에 대한 수량가중평균가를 최선 추정
    진입가로 반환한다. 근사(비-FIFO) — 백스톱 전용: 완전한 FIFO 로트
    추적은 이 코드베이스 어디에도 없고(정확한 경로는 실제 포지션 평단),
    이 함수는 coordinator에 접근할 수 없어 그 정확한 경로를 쓸 수 없다.
    entry 행이 하나도 없으면 None(호출자는 실현손익 기록을 스킵해야
    한다)."""
    try:
    ...
    for row in rows:
        if row.get("entry_or_exit") != "entry":
            continue
        qty = row.get("executed_quantity") or 0
        price = row.get("price")
...
async def reconcile_trade_ledger(kiwoom: Any, storage: Any, trade_date: str) -> dict:
    ...
    Returns:
        ...
        - checked: 이번 대조에서 살펴본 (주문번호, 종목코드) 쌍의 수 —
          브로커가 ord_no를 재사용해 서로 다른 종목에 같은 번호를 붙였다면
          그 각각이 독립적으로 1건씩 집계된다.
        - missing_orders: 원장 누적이 브로커 누적에 못 미쳐 새 행을 append한
          (ord_no, stk_cd) 쌍의 수(부분 부족 + 완전 누락 모두 포함).
    ...
    try:
        ...
        # 브로커 스냅샷을 (ord_no, stk_cd) 복합키로 누적 집계 (수량/금액/
        # 종목명/방향). ord_no 단독이 아닌 이유는 모듈 docstring 참조 —
        # 브로커의 ord_no 재사용으로 인한 타종목 오염 방지(F3 M1a와 동일
        # 계약).
        ...
        # 원장의 당일 행만, order_id가 있는 것만 (order_id, stk_cd)별로
        # 누적 집계 (브리프 명세: SELECT order_id, SUM(executed_quantity)
        # ... GROUP BY order_id — 리뷰 지적으로 stk_cd를 그룹 키에 추가해
        # 브로커 집계 키와 대칭을 맞췄다. WHERE는 trade_date — created_at은
        # KST datetime.now()로 삽입되므로 문자열 접두사 매칭으로 필터한다,
        # eod_snapshot과 동일 컨벤션).
        ledger: dict[tuple[str, str], dict[str, float]] = {}
        for row in ledger_rows:
            ...
            if not created_at.startswith(trade_date):
                continue
            key = (order_id, row.get("stk_cd"))
            entry = ledger.setdefault(key, {"qty": 0, "amount": 0.0})
            entry["qty"] += row.get("executed_quantity") or 0
            entry["amount"] += row.get("total_krw") or 0

        for key, b in broker.items():
            ord_no, stk_cd = key
            ledger_entry = ledger.get(key, {"qty": 0, "amount": 0.0})
            ...
            try:
                ...
                # record_trade_fill_async 자체는 never-raise 계약(내부에서
                # 스토리지 실패를 잡아 로그만 남기고 삼킨다)이라 이 except는
                # 사실상 도달하지 않는다 — 그 계약이 나중에 바뀌더라도 이
                # 루프가 한 주문의 쓰기 실패로 절대 깨지지 않도록 방어적으로
                # 유지한다.
                logger.warning(
                    f"[LedgerReconcile] ledger append failed for "
                    f"ord_no={ord_no} stk_cd={stk_cd}: {e}"
                )
                continue
