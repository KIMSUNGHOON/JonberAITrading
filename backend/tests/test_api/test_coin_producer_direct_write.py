...
세션 SSOT 통합 P2-4. `coin/analysis.py`가 legacy dict(B) + SessionManager
(SM) + `app.core.analysis_limiter.active_sessions`(3번째 사본)로 이중/삼중
쓰기하던 것을 SM 직접쓰기 단독으로 전환한다(legacy dict는 P3-1에서 완전
삭제됨). P2-3(KR, `e38a4ff`)과 동일 원칙, coin 코드에 적용한 것 — 이
파일은 REFACTOR 전 실패(RED)해야 하는, 목표 거동을 고정하는 특성화
테스트다:
