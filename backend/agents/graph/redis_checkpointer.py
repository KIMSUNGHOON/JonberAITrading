...
class RedisCheckpointer(BaseCheckpointSaver):
    ...
    Stores workflow state in Redis for:
    ...
    def _make_key(self, thread_id: str, checkpoint_id: Optional[str] = None) -> str:
        ...
        if checkpoint_id:
            return f"{self.session_id}:{thread_id}:{checkpoint_id}"
        return f"{self.session_id}:{thread_id}"
...
def create_redis_checkpointer(session_id: str) -> RedisCheckpointer:
    """
    Create a Redis checkpointer for a session.
    ...
    """
    return RedisCheckpointer(session_id)
