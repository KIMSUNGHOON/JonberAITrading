...
- **New `ReasoningWire`** — a **presentation-only** component. Props: `entries: string[]`, `running: boolean`, `currentStage?: string`, `emptyLabel?: string`. Its render core is **extracted from the dashboard `ReasoningPanel` tile** (numbered line prefix, `[Agent]`-prefix colorize via `prefixColor`, pulsing running head, auto-scroll to newest, dense mono). Location: `src/components/common/ReasoningWire.tsx` (shared across the tile and the page).
