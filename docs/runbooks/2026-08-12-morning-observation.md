...
### ⭐ 레짐 라벨이 결과에 도달하지 못한다 — 4일치로 확증 (2026-08-13 갱신)
...
bull → bear → bull → bull인데 목표는 **단조 감소**한다. 라벨이 아니라
`vol_multiplier_min`이 지배하고 있고, 그 값은 매일 EOD에 패널이 스스로 내린다
(0.5 → 0.4 → 0.3). **4일 연속 라벨이 결과를 바꾼 적이 없다.**

🔴 **다음 관측**: 패널이 08-13 EOD에서 0.3을 더 내리는지. 내린다면 이것은
"레짐 인지"가 아니라 **단방향 축소 루프**다 — 목표가 8%에서 계속 내려가면
신규 진입이 구조적으로 막힌다. 08-14 판정에서 eff가 **오르는 날이 있는지**가 관건.
